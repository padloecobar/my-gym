...
- IndexedDB local persistence via `lib/db/index.ts` (stores: exercises, sets w/ date/date_ts/exerciseId/exerciseId_date indexes, sessions, settings, mirror); add/delete sets update sessions metadata; openDb retries without version on VersionError; thin adapter in `lib/db/service.ts`.
- Data hooks in `src/hooks/` (`useExercises`, `useSets`, `useSessions`, `useSettings`) wrap db service, subscribe to `DB_CHANGE_EVENT`, and expose CRUD + state; pages use hooks instead of direct DB calls.
